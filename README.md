...
| プレースホルダ | 置換内容 | 出現ファイル |
| --- | --- | --- |
| `変換ツール` | ブランド名（例: お金ツール工房） | index.html, config.js, manifest.webmanifest, privacy/terms/operator.html |
| `単位変換、カラーコード変換、料理の分量換算、JSON整形をまとめた無料ツール` | 説明文（meta description等） | index.html, config.js, manifest.webmanifest |
| `変換ツール｜ツール工房` | ページタイトル（`<title>` / OGP） | index.html |
...
sed -i 's/変換ツール/お金ツール工房/g; s|https://ai-kaihatsubu.github.io/converter-tools/|https://example.github.io/money-suite/|g' \
