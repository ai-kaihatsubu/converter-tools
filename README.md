...
| プレースホルダ | 置換内容 | 出現ファイル |
| --- | --- | --- |
| `変換ツール` | ブランド名（例: お金ツール置き場） | index.html, config.js, manifest.webmanifest, privacy/terms/operator.html |
| `単位変換、カラーコード変換、料理の分量換算、JSON整形をまとめた無料ツール` | 説明文（meta description等） | index.html, config.js, manifest.webmanifest |
| `変換ツール｜ツール置き場` | ページタイトル（`<title>` / OGP） | index.html |
| `https://ai-kaihatsubu.github.io/converter-tools/` | 公開URL（末尾`/`付き。例: `https://example.github.io/money-suite/`） | index.html, privacy/terms/operator.html |
| `converter-tools-v1` | Service Workerキャッシュ名（例: `money-suite-v1`） | sw.js |
...
sed -i 's/変換ツール/お金ツール置き場/g; s|https://ai-kaihatsubu.github.io/converter-tools/|https://example.github.io/money-suite/|g' \
